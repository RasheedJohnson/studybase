...
Chapters plus bilingual Concepts (9. Auflage, 2023; ISBN 978-3-17-039779-8). Source PDF ships in the package folder. Concepts cover Kapitel 1-12 (numbered chapters from the PDF Inhaltsverzeichnis). Vorwort (`id: "0"`) has no concept rows yet. Questions and definitions are not wired yet.
...
Explore remains the Expo starter tab (separate from the offline textbook flow). More Reusables components can be added via the CLI as screens need them. Hagemann questions / definitions and Concepts for Vorwort (`id: "0"`) remain deferred.
